...
func TestWatcher_EnvForcePolling(t *testing.T) {
	...
	if !w.IsPolling() {
		t.Fatal("expected watcher to be in polling mode when TKV_FORCE_POLLING is set")
	}
}
...
func TestWatcher_EnvForcePoll(t *testing.T) {
	// Test TKV_FORCE_POLL (alternative env var)
	t.Setenv("TKV_FORCE_POLL", "true")
	...
	if !w.IsPolling() {
		t.Fatal("expected watcher to be in polling mode when TKV_FORCE_POLL is set")
	}
}
