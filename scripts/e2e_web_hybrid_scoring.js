...
function runTkv(args, env) {
  const out = execFileSync('tkv', args, { env, encoding: 'utf8' });
...
async function main() {
  ...
  const tmpDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'tkv-web-hybrid-'));
  ...
  const insights = runTkv(['--robot-insights'], env);
  const pagerank = insights?.full_stats?.pagerank || {};

  const text = runTkv(['--search', 'auth', '--search-mode', 'text', '--search-limit', String(issues.length), '--robot-search'], env);
  const hybrid = runTkv(['--search', 'auth', '--search-mode', 'hybrid', '--search-preset', 'impact-first', '--search-limit', String(issues.length), '--robot-search'], env);
