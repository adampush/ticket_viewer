...
TEXT_JSON=$(tkv --search "$QUERY" --search-limit "$LIMIT" --search-mode text --robot-search)
log "Text-only results (top IDs): $(echo "$TEXT_JSON" | jq -r '.results[].issue_id' | paste -sd ',' -)"

log "Running hybrid search (impact-first)..."
HYBRID_JSON=$(tkv --search "$QUERY" --search-limit "$LIMIT" --search-mode hybrid --search-preset impact-first --robot-search)
