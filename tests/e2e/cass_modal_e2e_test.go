...
{"id":"bv-002","title":"Another task","status":"in_progress","priority":2,"issue_type":"feature"}`
	...
	cmd.Env = append(os.Environ(),
		"TERM=xterm-256color",
		"TKV_TUI_AUTOCLOSE_MS=1000", // Auto-close after 1s
		"TKV_NO_CASS=1",             // Explicitly disable Cass lookup
	)
...
// TestCassDetectionEnvironmentVariable verifies that the TKV_NO_CASS environment
...
func TestCassDetectionEnvironmentVariable(t *testing.T) {
	...
	// With TKV_NO_CASS=1, Cass should be disabled
	cmd := scriptTUICommand(ctx, bv)
	cmd.Dir = tempDir
	cmd.Env = append(os.Environ(),
		"TERM=xterm-256color",
		"TKV_TUI_AUTOCLOSE_MS=1000",
		"TKV_NO_CASS=1",
	)
	...
	if err != nil {
		t.Fatalf("TUI failed with TKV_NO_CASS=1: %v\n%s", err, out)
	}
}
...
func TestCassModalRobotTriageNoCrash(t *testing.T) {
	...
	cmd.Dir = tempDir
	cmd.Env = append(os.Environ(), "TKV_NO_CASS=1")
