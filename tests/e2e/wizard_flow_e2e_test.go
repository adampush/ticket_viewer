...
func TestWizard_LocalExportFlow(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), "TKV_NO_BROWSER=1")
...
func TestWizard_DeployTargetSelection(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), "TKV_TEST_MODE=1", "TKV_NO_BROWSER=1")
...
func TestWizard_ExportConfigPrompts(t *testing.T) {
	...
	cmd.Dir = repoDir
	cmd.Stdin = strings.NewReader(input)
	cmd.Env = append(os.Environ(), "TKV_NO_BROWSER=1", "TKV_NO_SAVED_CONFIG=1")
